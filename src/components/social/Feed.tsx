...
export default function Feed({ setTab }: { setTab: (t: string) => void }) {
  ...
  const eliminarPublicacion = async (id: string) => {
    await fetch(`/api/business/publicaciones?id=${id}`, {
      method: 'DELETE'
    })
    ...
    <div className="flex gap-4">
        ...
        <div className="bg-white rounded-xl border border-[#e0dfdc] overflow-hidden">
          ...
          <div className="px-4 pb-4 -mt-6">
            ...
            {usuario?.foto
              ...
              : (
                <div className="w-12 h-12 rounded-full bg-[#0a66c2] border-2 border-white flex items-center justify-center text-white font-bold text-lg mb-2">
                  {usuario?.nombre[0]}
                </div>
              )
            }
      ...
      <div className="flex-1 flex flex-col gap-3 min-w-0">
        ...
        {publicaciones.length === 0 ? (
          ...
          <div
            key={i}
            className="bg-white rounded-xl border border-[#e0dfdc] p-4"
          >
            ...
            <div className="flex justify-between items-start mb-3">
              ...
              {/* BOTON ELIMINAR */}
              {String(p.autorId) === String(usuario?.id) && (
                <button
            ...
            <p className="text-sm text-[#000000e6] leading-relaxed whitespace-pre-wrap">
              {p.texto}
            </p>
      ...
      <aside className="w-64 flex-shrink-0 hidden xl:block">
        ...
        <div className="bg-white rounded-xl border border-[#e0dfdc] p-4 mb-3">
          ...
          {ofertas.slice(0, 3).map((o, i) => (
            <div
              key={i}
              className="mb-3 pb-3 border-b border-[#e0dfdc] last:border-0 last:mb-0 last:pb-0"
            >
          ...
          <button
            ...
            className="text-xs text-[#0a66c2] hover:underline mt-1"
          >
            Ver todas →
          </button>
